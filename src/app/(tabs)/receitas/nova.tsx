...
export default function NovaReceitaScreen() {
  ...
  function multiplicarQuantidades(fator: number) {
    setIngredientes((atual) =>
      atual.map((ingrediente) => ({
        ...ingrediente,
        quantidadeUsada: String(Number(ingrediente.quantidadeUsada.replace(',', '.')) * fator || 0),
      }))
    );
    setRendimento((atual) => String(Number(atual.replace(',', '.')) * fator || 0));
    ...
    <ThemedView style={styles.container}>
      <SafeAreaView style={styles.safeArea}>
        <ScrollView contentContainerStyle={styles.form}>
          <ThemedText type="subtitle">Nova receita</ThemedText>
          ...
          <View style={styles.field}>
            ...
            {ingredientes.length > 0 && (
              <View style={styles.acoesIngredientes}>
                ...
                <Pressable onPress={() => setMostrarOpcoes((atual) => !atual)}>
                  <ThemedText type="link">+ Opções</ThemedText>
                </Pressable>
              </View>
            )}
            ...
            {mostrarOpcoes && (
              <View style={styles.acoesIngredientes}>
                <Pressable onPress={() => multiplicarQuantidades(2)}>
                  <ThemedView type="backgroundElement" style={styles.opcaoButton}>
                    <ThemedText type="small">Duplicar quantidades (×2)</ThemedText>
                  </ThemedView>
                </Pressable>
                <Pressable onPress={() => multiplicarQuantidades(3)}>
                  <ThemedView type="backgroundElement" style={styles.opcaoButton}>
                    <ThemedText type="small">Triplicar quantidades (×3)</ThemedText>
                  </ThemedView>
                </Pressable>
              </View>
            )}
          ...
          <Pressable onPress={handleSalvar}>
            <ThemedView type="backgroundSelected" style={styles.salvarButton}>
              <ThemedText type="smallBold">Salvar receita</ThemedText>
            </ThemedView>
          </Pressable>
        </ScrollView>
      </SafeAreaView>
